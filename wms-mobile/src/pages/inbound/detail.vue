      ...
      <view class="info-card">
        ...
        <view class="info-list">
          ...
          <view class="info-item">
            ...
            <text class="value">{{ order.warehouseEntryNo || '-' }}</text>
          </view>
          <view class="info-item">
...
async function loadOrder() {
  ...
  try {
    const res = await inboundAPI.getById(orderId.value)
    order.value = res
  } catch (error: any) {
...
function handleConfirm() {
  ...
  uni.showModal({
    title: '确认入库',
    content: `确定要将订单 ${order.value.orderNo} 确认入库吗？`,
    success: async (res) => {
      if (res.confirm) {
        try {
          uni.showLoading({ title: '处理中...' })
          await inboundAPI.confirm(orderId.value, { source: 'mobile' })

  ...
  <view class="inbound-page">
    ...
    >
        ...
        >
          ...
          <view class="card-footer">
            ...
            <view class="action-btn" @click.stop="handleAction(order)">
              {{ order.status === 'pending' ? '确认入库' : '查看详情' }}
            </view>
          </view>
        </view>
      </view>
      ...
      <view class="empty-state" v-else-if="!loading">
        ...
        <text class="empty-text">暂无入库订单</text>
      </view>
...
// 操作按钮
async function handleAction(order: InboundOrder) {
  if (order.status === 'pending') {
    uni.showModal({
      title: '确认入库',
      content: `确定要将订单 ${order.orderNo} 确认入库吗？`,
      success: async (res) => {
        if (res.confirm) {
          try {
            uni.showLoading({ title: '处理中...' })
            await inboundAPI.confirm(order.id, { source: 'mobile' })

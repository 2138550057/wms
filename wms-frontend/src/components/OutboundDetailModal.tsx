...
const OutboundDetailModal: React.FC<OutboundDetailModalProps> = ({
  visible,
  orderId,
  onCancel,
  onEdit,
  ...
  const loadData = async () => {
    ...
    setLoading(true);
    try {
      const response: any = await outboundAPI.getById(orderId);
      if (response.success) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const response: any = await outboundAPI.confirm(order.id);
      if (response.success) {
  ...
  const handleReverseAudit = async () => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const response: any = await outboundAPI.reverseAudit(order.id);
          if (response.success) {
      ...
      >
        <Spin spinning={loading}>
          <div style={{ maxHeight: '70vh', overflow: 'auto' }}>
            ...
            >
              <Descriptions bordered column={3} size="small">
                ...
                <Descriptions.Item label="业务类型">
                  {order.businessType === 'sales' && '销售出库'}
                  {order.businessType === 'return' && '退货出库'}
                  {order.businessType === 'transfer' && '调拨出库'}
                </Descriptions.Item>
            ...
            {/* 出库明细 */}
            <Card
              title={<span style={{ fontSize: '14px', fontWeight: 600 }}>出库明细 ({order.items?.length || 0}条)</span>}
              size="small"
            >
              <Table
                columns={columns}

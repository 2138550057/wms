...
import { logAPI, OutboundLog } from '../../services/log.service';
...
const OutboundLogList: React.FC = () => {
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState<OutboundLog[]>([]);
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const response: any = await logAPI.getOutboundLogs({
        page,
        size,
        ...filters,
      });
  ...
  const handleExport = async () => {
    try {
      ...
      if (response.success) {
        const excelData = response.data.map((item: OutboundLog) => ({
          '出库日期': dayjs(item.outboundDate).format('YYYY-MM-DD'),
          '出库单号': item.orderNo,
          '进仓编号': item.warehouseEntryNo || '',
          '客户名称': item.customerName,
          '业务类型':
            item.businessType === 'sales'
              ? '销售出库'
          ...
          '收货单位': item.receivingCompany || '',
          '收货地址': item.receivingAddress || '',
          '车牌号': item.vehicleNumber || '',
          '司机': item.driverName || '',
        ...
        XLSX.utils.book_append_sheet(wb, ws, '出库日志');
        XLSX.writeFile(wb, `出库日志_${dayjs().format('YYYYMMDD_HHmmss')}.xlsx`);
...
export default OutboundLogList;
